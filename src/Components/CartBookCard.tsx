...
function CartBookCard({book,index}:{book:any,index:number}){
    ...
    return(
        <div className="flex gap-10 w-[300px] h-[150px] rounded font-[Roboto]">
            ...
            {index===-1?<div className='flex gap-1 items-center'>Quatity: <div className='w-[40px] h-[28px] text-center border-2 rounded'>{book.quantityToBuy}</div></div>:<div className='flex gap-1 items-center ml-[-10px]'><IconButton onClick={handleRemove} disabled={book.quantityToBuy===1?true:false}><RemoveCircleOutline/></IconButton><div className='w-[40px] h-[28px] text-center border-2 rounded'>{book.quantityToBuy}</div><IconButton onClick={handleAdd} disabled={book.quantityToBuy<book.quantity?false:true}><AddCircleOutline/></IconButton><Button onClick={removeItem}>Remove</Button></div>}
            </div>
        </div>
    )
}
